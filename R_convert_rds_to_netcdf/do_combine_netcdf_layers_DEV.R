...
infile <- file.path(gsub("data_derived", "working_ivan", rootdir), "bushfiresmoke_v1_3_2001_2020_remainder_p99.nc")
system2("gdalinfo", infile)
...
# infile <- file.path(rootdir, paste0("bushfiresmoke_v1_3_2001_2020_", var_i, "_out.nc"))
infile <- file.path(gsub("data_derived", "working_ivan", rootdir), paste0("bushfiresmoke_v1_3_2001_2020_", var_i, "_compressed.nc"))
system2("gdalinfo", infile)
r0 <- terra::rast(infile)
r0
plot(r0[[1:4]])
r1 <- ifel(r0[[1:1000]] > r99, r99, r0[[1:1000]])
writeCDF(r1, filename = file.path("working_ivan", paste0("bushfiresmoke_v1_3_2001_2020_",var_i,"_trimmed.nc")))

r1 <- terra::rast(file.path("working_ivan", paste0("bushfiresmoke_v1_3_2001_2020_",var_i,"_trimmed.nc")))
#crs(r1)  <- "epsg:3577"
#time(r1) <- seq(as.Date("2001-01-01"), as.Date("2003-09-27"), by = "day")
stars_r1 <- stars::st_as_stars(r1, ignore_file = T)
dim(stars_r1)
names(stars_r1) <- var_i
stars::write_mdim(stars_r1, file.path("working_ivan", paste0("bushfiresmoke_v1_3_2001_2020_",var_i,"_trimmed_2.nc")))

r1 <- terra::rast(file.path("working_ivan", paste0("bushfiresmoke_v1_3_2001_2020_",var_i,"_trimmed_2.nc")))
...
len <- length(e)
len
plot(1:len,e, type = "b", ylim = c(-15, 80))
#outfile <- file.path(rootdir, paste0("bushfiresmoke_v1_3_2001_2020_", var_i, "_trimmed_p99.rds"))
#saveRDS(r0, outfile)

infile <- file.path(gsub("data_derived", "working_ivan", rootdir), paste0("bushfiresmoke_v1_3_2001_2020_", var_i, "_compressed.nc"))
# system2("gdalinfo", infile)
r_raw <- terra::rast(infile)
r_raw
er_raw <- extract(r_raw, xy)
lenr_raw <- length(er_raw)
lenr_raw
points(1:lenr_raw,er_raw, pch = 16, cex = 0.7)
